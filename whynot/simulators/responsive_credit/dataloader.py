...
class DataLoader(object):
    ...
    def __init__(self, datafile="credit_data.zip", seed=None):
        cur_dir = os.path.abspath(os.path.abspath(''))
    ...
    def feature_names(self):
        """Return the names of the dataset features"""
        if self._features is None:
            self._features, self._labels, self._info = self.load_data()
        return np.copy(self._info["feature_names"])
